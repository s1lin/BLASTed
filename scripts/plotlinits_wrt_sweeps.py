...
def plotquantity(filelist, quantname, numits, labellist, opts, imageformatstring, num_per_set):
    plt.close()
    markdivisor = 1
    for i in range(len(filelist)):
        ...
        quotientindex = i // num_per_set

        # number of points to plot
        pltdatalen = len(data[:,0])-numits

        plt.xlabel("Sweeps", fontsize="medium")
        if quantname == "linits":
            plt.plot(data[numits:,0], data[numits:,1], \
                    lw=opts['linewidth'], ls=opts['linetype'][i], color=opts['colorlist'][quotientindex], \
                    marker=opts['marklist'][i], ms=opts['marksize'], \
                    mew=opts['markedgewidth'], \
                    #markevery=list(range(0,pltdatalen,opts['markinterval'])), \
                    label=labellist[i])
            plt.ylabel("Linear solver iterations required", fontsize="medium")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_per_set", type=int, help = "Number of consecutive files to use the same mark symbol for")
    ...
    plotquantity(args.files[1:], "linits", args.start_iter, args.labels, opts, args.format, args.num_per_set)
